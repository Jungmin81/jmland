...
file_train = open(r'C:\Users\CHEON\Desktop\mteg\mask-detection\train.txt', 'w')
file_test = open(r'C:\Users\CHEON\Desktop\mteg\mask-detection\test.txt', 'w')

counter = 1
index_test = round(100 / percentage_test)

for pathAndFilename in glob.iglob(os.path.join(img_dir, "*.png")):
    title, ext = os.path.splitext(os.path.basename(pathAndFilename))

    if int(counter) == int(index_test):
        counter = 1
        file_test = open(r'C:\Users\CHEON\Desktop\mteg\mask-detection/train_test/test.txt', 'a')
        file_test.write(img_dir + "/" + title + '.png' + "\n")
        file_test.close()
    else:
        file_train = open(r'C:\Users\CHEON\Desktop\mteg\mask-detection/train_test/train.txt', 'a')
